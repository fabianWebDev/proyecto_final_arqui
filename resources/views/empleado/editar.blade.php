        ...
        <div class="w-full mx-auto p-1">
            <div class="text-gray-900 dark:text-gray-100">
                <div class="bg-white dark:bg-gray-800 rounded-lg dark:divide-gray-900 p-3">
                    <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">Editar Colaborador</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        {{ __("Edita un colaborador.") }}
                    </p>
                    <form class="w-full mt-4" method="POST" action="{{ route('empleado.update', $empleado) }}">
                        @csrf @method('PUT')
                        <div class="flex flex-wrap -mx-3 mb-6">
                            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                <x-input-label for="nombre" value="Nombre" />
                                <x-text-input id="nombre" name="nombre" type="text" class="mt-1 block w-full" :value="old('nombre', $empleado->nombre)" required autofocus autocomplete="nombre" />
                                <x-input-error :messages="$errors->get('nombre')" class="mt-2" />
                            </div>
                            <div class="w-full md:w-1/2 px-3">
                                <x-input-label for="fecha_ingreso" value="Fecha de Ingreso" />
                                <input class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" id="fecha_ingreso" type="date" value="{{ old('fecha_ingreso', $empleado->fecha_ingreso)}}" name="fecha_ingreso">
                        ...
                        <div class="flex flex-wrap -mx-3 mb-6">
                            <div class="w-full md:w-1/2 px-3">
                                ...
                                <input class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" id="correo_electronico" type="email" value="{{ old('correo_electronico', $empleado->correo_electronico )}}" name="correo_electronico">
                                <x-input-error :messages="$errors->get('correo_electronico')" class="mt-2" />
                            </div>
                            <div class="w-full md:w-1/2 px-3">
                                <x-input-label for="cedula" value="Cédula" />
                                <input class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" id="cedula" type="number" value="{{ old('cedula', $empleado->cedula)}}" name="cedula">
                        ...
                        <div class="flex flex-wrap -mx-4 mb-2">
                            ...
                            <div class="w-full md:w-1/4 px-3 mb-6 md:mb-0">
                                ...
                                <select name="id_perfil" id="id_perfil" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    <option value="{{ $perfil->id }}" @if($perfil->id == $empleado->perfil->id) selected @endif>{{ $perfil->nombre }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mt-4 mb-2">
                            <x-primary-button>Editar Colaborador</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
